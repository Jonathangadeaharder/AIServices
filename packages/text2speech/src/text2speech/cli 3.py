...
def generate(
    ...
    reference_audio: str | None = typer.Option(
        None, "--reference-audio", "-ra", help="Path to reference audio for voice cloning"
    ),
    reference_text: str | None = typer.Option(
        None, "--reference-text", "-rt", help="Transcript of the reference audio"
    ),
    emotion: str | None = typer.Option(
        None, "--emotion", "-e", help="Emotion tag (e.g. 'happy', 'whisper')"
    ),
