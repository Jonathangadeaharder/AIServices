...
def test_generate_with_options(mocker, tmp_path):
    ...
    result = runner.invoke(
        app,
        ["--prompt", "test", "--output", str(out), "--seconds", "4", "--fps", "24", "--width", "704", "--height", "480", "--steps", "8"],
    )
    assert result.exit_code == 0
    ...
    assert mock_provider.last_request.num_inference_steps == 8
